...
This is a subset documentation on the aws static website project, built on Github Actions.
It focuses on describing the DeployUser tepology and it's downsides compared to OIDC
...
<!-- TOC -->
- [DeployUserAccessKey](#deployuseraccesskey)
...
* **AWS IAM**
  ...
  * OIDC is NOT used here
  * Dedicated deploy user with least-privilege permissions is created
    * BucketPolicy to allow this user Update write access
    * DistributionPolicy enabled this user invalidating Cache
  * aws cli (CloudShell) is used to generate Access Keys for this user, the **keys are only displayed once**.
    * The Access keys are then manually pasted into GitHub Secrets
    * It is also possible (but even worse) to generate and Store the AccessKeys within the CLoudFormation Stack, the keys would be **permanently visible** in the Stack overview.

* **GitHub**

  * Stores deployment secrets
  * Source code repository
  * CI/CD via GitHub Actions (access through keys stored in secrets)

* **AWS S3**

  * **Write Access through DeployUser**
  * Stores static build artifacts
  * No public access, no website configuration

* **AWS CloudFront**

  * **Distribution can be invalidated through DeployUser**
...
The overview shows 110_deployuser_accesskeys.cf.yml resources with simplified relations when creating a deploy user and hosting the actual build on GitHub Actions.
...
manual AWS console configuration is required beyond stack creation: paste credentials from Stack Output to GitHub Secrets
...
(removed generic information about the project)
